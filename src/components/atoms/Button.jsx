...
export default function Button({ as = 'button', href, onClick, children = 'Contact us', className = '', variant = 'primary', size = 'lg', fullWidth = false, disabled = false, loading = false, leftIcon = null, rightIcon = null, ariaLabel, type = 'button' }) {
  ...
  return (
    <Comp href={href} type={href ? undefined : type} onClick={onClick} aria-label={ariaLabel || (typeof children === 'string' ? children : 'button')} disabled={disabled || loading} className={cn('group', base, VARIANTS[variant] || VARIANTS.primary, SIZES[size] || SIZES.md, fullWidth ? 'w-full' : 'w-auto', disabled ? 'opacity-60 cursor-not-allowed' : 'cursor-pointer', className)} initial={{ y: 0, scale: 1, rotate: 0 }} whileHover={{ y: -2, scale: 1.02, rotate: 0.2 }} whileTap={{ scale: 0.98, y: 0 }} transition={spring}>
