  ...
  return (
    <motion.header
      ...
      <div className={cn(' container ', 'transition-[padding] duration-300', scrolled ? '!py-3' : '!py-5')}>
        <div className={cn('flex items-center justify-between rounded-xl', 'transition-all duration-300', scrolled ? ' !py-3 !px-3 backdrop-blur-md bg-white/70 border border-black/[.06] shadow-[0_8px_24px_rgba(0,0,0,.08)]' : '')}>
          {/* Left: Logo */}
          ...
          <>
            ...
            <motion.aside className='fixed right-0 top-0 z-50 h-full w-[86%] max-w-sm bg-white shadow-2xl' initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', stiffness: 380, damping: 38 }}>
              ...
              <div className='px-4 pt-2'>
                <Link href='#contact-us' onClick={() => setOpen(false)} className='inline-flex w-full items-center justify-center h-12 rounded-full bg-[#4A55A6] text-white font-semibold hover:opacity-95 transition'>
                  Contact us
                </Link>
              </div>
            </motion.aside>
